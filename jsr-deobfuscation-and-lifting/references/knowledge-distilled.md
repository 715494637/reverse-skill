...
- Lift only the replay-critical path; whole-bundle beautification is usually waste.
...
- Spending time on pretty-print cleanup that does not improve replay or tracing.
...
## Evidence Standard
- One explicit protected entry path.
- One semantic stage map on the critical path.
- One equivalence check between original and lifted behavior.
- One recovered artifact that can feed replay, locate, or protocol work directly.
