...
|---|---|---|---|
| Webpack | Bootstrap loader, chunk push, module cache, module id resolution | Which module actually owns the critical path | Reading helper modules before proving ownership |
| Worker | `new Worker`, blob URL creation, worker script URL, `importScripts`, `postMessage` shape | Whether the critical logic lives in the worker or only receives built data | Lifting worker internals before capturing the thread boundary |
...
- Apply reversible transforms only on the critical analysis path.
- Stop once lifted semantics can support later locate, stage-analysis, or protocol work directly.
