...
验证前门 skill 面对瑞数场景时，是否会把任务识别成 `首跳挑战 -> cookie/state 产出 -> 二跳消费` 的两阶段链路，并读取瑞数专用 reference，而不是把首跳脚本或首个 cookie 写入当成完成。
...
```text
$jsr-reverse 这是一个 RS/瑞数风格的 412 -> cookie/state -> 200 两跳链路。先按瑞数路线定位首跳产物和二跳消费，再决定是否进入 recover 或 runtime。
```
